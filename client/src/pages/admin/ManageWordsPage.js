            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
                ...
                <div className="flex items-center gap-4 w-full sm:w-auto">
                    ...
                    <Link 
                        ...
                        className="bg-gray-100 text-gray-800 font-bold px-4 py-2.5 rounded-lg flex items-center gap-2 hover:bg-gray-200 flex-shrink-0"
                    >
                        <span>Pratinjau</span>
                    </Link>
                    <button 
                        onClick={() => setFormModalState({ isOpen: true, mode: 'add', data: null })} 
                        className="bg-green-500 text-white font-bold px-4 py-2.5 rounded-lg flex items-center gap-2 hover:bg-green-600 flex-shrink-0"
                    >
                        <PlusIcon />
                        <span>Tambah</span>
                    </button>
            ...
            <div className="bg-white rounded-xl shadow-md overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-left min-w-[800px]">
                        ...
                        <tbody>
                            {isLoading ? (
                                <tr style={{ height: `${ITEMS_PER_PAGE * 70}px` }}>
                                ...
                                <tr style={{ height: `${ITEMS_PER_PAGE * 70}px` }}>
                                    <td colSpan="7" className="text-center align-middle text-red-500">{error}</td>
                                </tr>
                            ) : (
                                <>
                                    {/* Actual data rows */}
                                    {currentItems.map((entry, index) => (
                                        <tr key={entry._id} className="border-b border-gray-200 hover:bg-gray-50" style={{ height: '70px' }}>
                                    ...
                                    {emptyRows.map((_, index) => (
                                        <tr key={`empty-${index}`} className="border-b border-gray-200" style={{ height: '70px' }}>
                                            <td className="p-4"></td>
                                            <td className="p-4"></td>
                                            <td className="p-4"></td>
                                            <td className="p-4"></td>
                                    ...
                                    {currentItems.length === 0 && (
                                        <tr style={{ height: `${ITEMS_PER_PAGE * 70}px` }}>
                                            <td colSpan="7" className="text-center align-middle text-gray-500">Belum ada kosakata di topik ini.</td>
                                        </tr>
                                    )}
                                </>
                            )}
