...
const Sidebar = () => {
    // 2. Panggil hook yang diperlukan
    const { logout } = useAuth();
    const navigate = useNavigate();

    const handleLogout = () => {
        logout();
        navigate('/admin/login'); // Arahkan ke login admin setelah logout
    };

    return (
        // Gunakan flexbox untuk mengatur posisi tombol logout di bawah
        <div className="w-24 bg-white h-screen flex flex-col items-center py-6 shadow-lg fixed">
            <div>
                <Link to="/admin/dashboard">
                    <img src={logo} alt="Logo" className="h-12 w-auto" />
                </Link>
                <nav className="flex flex-col items-center mt-12 space-y-6">
                    <Link to="/admin/dashboard" className="p-3 rounded-lg bg-blue-100 text-blue-600">
                        <HomeIcon />
                    </Link>
                    <Link to="#" className="p-3 rounded-lg text-gray-500 hover:bg-gray-100">
                        <BookIcon />
                    </Link>
                    <Link to="#" className="p-3 rounded-lg text-gray-500 hover:bg-gray-100">
                        <UsersIcon />
            ...
            {/* 3. Tambahkan tombol logout di sini */}
            {/* `mt-auto` akan mendorong tombol ini ke bagian paling bawah */}
            <button 
                onClick={handleLogout} 
                className="mt-auto p-3 rounded-lg text-red-500 hover:bg-red-100"
                title="Logout"
            >
                <LogoutIcon />
            </button>
